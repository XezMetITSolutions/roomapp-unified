...
// Connection pool ayarlar─▒ ve retry logic
const connectWithRetry = async (retries = 10) => {
  for (let i = 0; i < retries; i++) {
    try {
      await prisma.$connect()
      console.log('Γ£à Database connected successfully')
      return
    } catch (error) {
      console.error(`Γ¥î Database connection attempt ${i + 1}/${retries} failed:`, error)
      if (i === retries - 1) {
        console.error('Γ¥î All database connection attempts failed')
        throw error
      }
      // Exponential backoff: 2s, 4s, 8s, 16s, 32s, 64s, 128s, 256s, 512s
      const delay = Math.min(2000 * Math.pow(2, i), 30000)
      console.log(`ΓÅ│ Waiting ${delay}ms before next attempt...`)
...
connectWithRetry().catch((error) => {
  console.error('Γ¥î Failed to connect to database:', error)
  console.log('ΓÜá∩╕Å Server will continue without database connection')
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    if (origin.endsWith('.roomxqr.com') || origin === 'https://roomxqr.com') {
...
app.post('/api/admin/tenants', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // Slug'─▒ temizle ve kontrol et
    const cleanSlug = slug.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '')
    // Tenant'─▒n zaten var olup olmad─▒─ƒ─▒n─▒ kontrol et
    const existingTenant = await prisma.tenant.findUnique({
      where: { slug: cleanSlug }
    })
    if (existingTenant) {
      res.status(400).json({ message: 'Bu slug zaten kullan─▒l─▒yor' }); return;
    }
    // Tenant olu┼ƒtur
    ...
    const hotel = await prisma.hotel.create({
      data: {
        name: `${name} Otel`,
        address: 'Adres bilgisi g├╝ncellenmeli',
        phone: 'Telefon bilgisi g├╝ncellenmeli',
        email: adminEmail,
        tenantId: tenant.id
      }
    })
    // ─░lk admin kullan─▒c─▒ olu┼ƒtur
    ...
    res.status(201).json({
      message: 'Tenant ba┼ƒar─▒yla olu┼ƒturuldu',
      tenant: {
        id: tenant.id,
        name: tenant.name,
        slug: tenant.slug,
...
app.post('/api/admin/tenants/:id/features', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // TypeScript'e de─ƒerlerin string oldu─ƒunu garanti et
    const validId: string = id
    const validFeatureKey: string = featureKey

    // Tenant'─▒n var olup olmad─▒─ƒ─▒n─▒ kontrol et
    const tenant = await prisma.tenant.findUnique({
      where: { id: validId }
    })

    if (!tenant) {
      res.status(404).json({ message: 'Tenant bulunamad─▒' }); return;
    }

    // ├ûzelli─ƒi olu┼ƒtur veya g├╝ncelle
    ...
    res.json({ 
      message: '├ûzellik ba┼ƒar─▒yla g├╝ncellendi',
      feature 
    }); return;
  } catch (error) {
...
app.post('/api/admin/features/bulk-update', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (!tenantIds || !Array.isArray(tenantIds) || !featureKey) {
      res.status(400).json({ message: 'Ge├ºersiz parametreler' }); return;
    }
    ...
    res.json({ 
      message: 'Toplu g├╝ncelleme tamamland─▒',
      results 
    }); return;
  } catch (error) {
    ...
    const availableFeatures = [
      {
        key: 'qr-menu',
        name: 'QR Men├╝',
        description: 'QR kod ile men├╝ eri┼ƒimi',
        category: 'temel'
      },
      {
        key: 'multi-language',
        name: '├çoklu Dil Deste─ƒi',
        description: 'Birden fazla dil deste─ƒi',
        category: 'temel'
      },
      {
        key: 'analytics',
        name: 'Analitik',
        description: 'Detayl─▒ analitik raporlar',
        category: 'geli┼ƒmi┼ƒ'
      },
      {
        key: 'custom-branding',
        name: '├ûzel Markalama',
        description: 'Logo ve tema ├╢zelle┼ƒtirmesi',
        category: 'geli┼ƒmi┼ƒ'
      },
      {
        key: 'api-access',
        name: 'API Eri┼ƒimi',
        description: 'REST API eri┼ƒimi',
        category: 'geli┼ƒmi┼ƒ'
      },
      {
        key: 'priority-support',
        name: '├ûncelikli Destek',
        description: '7/24 ├╢ncelikli m├╝┼ƒteri deste─ƒi',
        category: 'destek'
      },
      {
        key: 'custom-integrations',
        name: '├ûzel Entegrasyonlar',
        description: '├£├º├╝nc├╝ parti sistem entegrasyonlar─▒',
        category: 'geli┼ƒmi┼ƒ'
      },
      {
        key: 'advanced-notifications',
        name: 'Geli┼ƒmi┼ƒ Bildirimler',
      ...
      {
        key: 'backup-restore',
        name: 'Yedekleme ve Geri Y├╝kleme',
...
server.listen(PORT, async () => {
  ...
  console.log(`≡ƒô▒ Frontend URL: ${process.env.FRONTEND_URL}`)
  console.log(`≡ƒùä∩╕Å Database: ${process.env.DATABASE_URL?.split('@')[1]}`)
  
  // Super admin olu┼ƒtur
  try {
    await createSuperAdmin()
    console.log('Γ£à Super admin haz─▒r')
  } catch (error) {
    console.error('Γ¥î Super admin olu┼ƒturma hatas─▒:', error)
  }
})
