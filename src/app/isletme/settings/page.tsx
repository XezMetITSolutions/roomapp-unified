...
export default function SettingsPage() {
  ...
  const handleAutoColorGeneration = () => {
    console.log('Otomatik renk ayarı başlatıldı');
    
    // Rastgele bir renk kombinasyonu seç
    const randomIndex = Math.floor(Math.random() * predefinedColorSchemes.length);
    const selectedScheme = predefinedColorSchemes[randomIndex];
    
    console.log('Seçilen renk kombinasyonu:', selectedScheme);
    
    // Gradient renkler oluştur
    const gradientColors = generateGradientColors(selectedScheme.primary, selectedScheme.secondary);
    
    // HSL dönüşümü için yardımcı fonksiyon
    const hexToHsl = (hex: string) => {
      const r = parseInt(hex.slice(1, 3), 16) / 255;
      const g = parseInt(hex.slice(3, 5), 16) / 255;
      const b = parseInt(hex.slice(5, 7), 16) / 255;
    ...
    // Arka plan renklerini kullanıcının seçtiği tema moduna göre belirle
    // Otomatik ayarla butonu tema modunu değiştirmemeli, sadece renkleri değiştirmeli
    ...
    // Seçilen renk kombinasyonunun adını kaydet
    setSelectedColorScheme(selectedScheme.name);
  ...
  const [selectedColorScheme, setSelectedColorScheme] = useState<string>('Lüks Otel');
                      ...
                      <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                          ...
                          <button
                            ...
                            className="flex items-center justify-center space-x-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-300 shadow-md hover:shadow-lg hover:scale-105 active:scale-95"
                          >
                        ...
                        <p className="text-sm text-gray-600">
                          Rastgele güzel renk kombinasyonlarından birini seçer ve otomatik olarak uygular.
                        </p>
                        {selectedColorScheme && (
                          <div className="mt-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                            <p className="text-sm font-medium text-blue-900">
                              Aktif Tema: <span className="font-semibold">{selectedColorScheme}</span>
                            </p>
                          </div>
                        )}
                      </div>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setThemeSettings(prev => ({
                                    ...
                                    // Tema moduna göre arka plan renklerini güncelle
